...
public class P {
		...
		while(true) {
			...
			if(num == 1) {
	         ...
	         //2. 가격 변경
					...
					System.out.print("가격을 변경하실 메뉴의 번호를 입력 하세요(1~" + count + ") :");
					int i = input.nextInt();
			...
			//5. 수익 확인
					...
					System.out.println("현재까지 수익은 " + profit + "입니다.");
				}
	...
	public static void user() {
		...
		while(true) {
			...
			//관리자 비밀번호는 1004. 입력하면 관지라 함수로 넘어감.
			if(money == 1004) {
				admin();
				return;
			}
			...
			if(money >= price[num-1]) {
				...
				//돈이 남아있다면 선택지를 준다.
				System.out.println("1. 계속 구매 2. 금액 추가 3. 잔돈 반환");
				System.out.print("번호를 입력하세요.");
